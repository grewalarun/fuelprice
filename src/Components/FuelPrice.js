  ...
  componentDidMount() {
    axios({
      ...
      headers: {
        ...
        "x-rapidapi-host": "newsrain-petrol-diesel-prices-india-v1.p.rapidapi.com",
//        "x-rapidapi-key": "0c3812ba95msh17c9947266672ddp16f018jsn0289a3c20744",
  ...
  render() {
    const urcity = this.state.cities.filter(
      (a) => a.state === this.state.defaultState && a.city === this.state.defaultCity
    );
    ...
    urcity.map((u) => (
            <Container>
              ...
              <ul className="staeslist">
                 {  allstate.map((c,i) => (
                 <li key={i} onClick={()=>this.handleClick(c.state,c.city)}>{c.state}<span>{c.city}</span></li>
                 ))
                 }
                 </ul>
            </Container>
